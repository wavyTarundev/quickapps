...
const Index = ({ configData, storeDetails, landingPageData }) => {
  ...
  const metaTitle = `${storeDetails?.meta_title ? storeDetails?.meta_title : storeDetails?.name
    } - ${configData?.business_name}`;
  const metaImage = `${configData?.base_urls?.store_image_url}/${storeDetails?.meta_image ?? storeDetails.cover_photo
    }`;
